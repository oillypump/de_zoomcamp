...
def ingest_data(
    url: str, engine, target_table: str, chunksize: int = 100000
) -> pd.DataFrame:
    df_iter = pd.read_csv(
...
@click.command()
@click.option("--pg-user", envvar="pg_user", default="root", help="PostgreSQL username")
@click.option("--pg-pass", envvar="pg_pass", default="root", help="PostgreSQL password")
@click.option(
    "--pg-host", envvar="pg_host", default="localhost", help="PostgreSQL host"
)
@click.option("--pg-port", envvar="pg_port", default=5432, help="PostgreSQL port")
@click.option(
    "--pg-db", envvar="pg_db", default="ny_taxi", help="PostgreSQL database name"
)
...
def main(
    pg_user, pg_pass, pg_host, pg_port, pg_db, year, month, chunksize, target_table
):
    print(f"Connecting to Postgres at {pg_host}:{pg_port}, database: {pg_db}")
